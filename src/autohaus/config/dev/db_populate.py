...
from autohaus.config.config import resources_path
...
from autohaus.config.dev_modus import dev_db_populate
from autohaus.repository import engine
...
class DbPopulateService:
    ...
    def _load_csv_files(self) -> None:
        logger.debug("begin")
        tabellen: Final = ["autohaus", "adresse", "auto"]
        csv_path: Final = "/init/autohaus/csv"
        # siehe extras/compose/postgres/compose.init.yml
        with self.engine_admin.connect() as connection:
            connection.execute(text("SET search_path TO autohaus;"))
