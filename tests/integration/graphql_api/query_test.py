...
def test_query_id() -> None:
    ...
    autohaus: Final = data["autohaus"]
    assert isinstance(autohaus, dict)
...
def test_query_id_notfound() -> None:
    ...
    response_body: Final = response.json()
    assert isinstance(response_body, dict)
    assert response_body["data"]["autohaus"] is None
    assert response_body.get("errors") is None
...
def test_query_email() -> None:
    ...
    response_body: Final = response.json()
    assert isinstance(response_body, dict)
    autohaeuser: Final = response_body["data"]["autohaeuser"]
    assert isinstance(autohaeuser, list)
    assert len(autohaeuser) > 0
    assert response_body.get("errors") is None
...
def test_query_email_notfound() -> None:
    ...
    query: Final = {
        "query": """
            {
                autohaeuser(suchparameter: {email: "not.found@acme.com"}) {
                    id
                }
            }
        """,
...
def test_query_name() -> None:
    ...
    query: Final = {
        "query": """
            {
                autohaeuser(suchparameter: {name: "Auto"}) {
                    id
                    version
                    name
                    email
...
def test_query_name_notfound() -> None:
    ...
    query: Final = {
        "query": """
            {
                autohaeuser(suchparameter: {name: "Nichtvorhanden"}) {
                    id
                }
            }
        """,
